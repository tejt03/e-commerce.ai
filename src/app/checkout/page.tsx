  ...
  if (buyNow) {
    if (!Number.isFinite(productId) || productId <= 0) {
      return (
        <main className="min-h-screen px-4 py-10">
          <section className="mx-auto max-w-3xl rounded-2xl border bg-white/90 p-6 shadow-sm">
            <h1 className="text-2xl font-semibold text-slate-900">Checkout</h1>
            <p className="mt-2 text-sm text-red-600">Invalid product.</p>
            <Link
              href="/products"
              className="mt-4 inline-block rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700"
            >
    ...
    if (error || !p) {
      return (
        <main className="min-h-screen px-4 py-10">
          <section className="mx-auto max-w-3xl rounded-2xl border bg-white/90 p-6 shadow-sm">
            <h1 className="text-2xl font-semibold text-slate-900">Checkout</h1>
            <p className="mt-2 text-sm text-red-600">Product not found.</p>
            <Link
              href="/products"
              className="mt-4 inline-block rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700"
            >
    ...
    if (cart?.id) {
      const { data } = await supabase
        .from("cart_items")
        .select(
          "product_id, quantity, products!inner(id,title,price,image_url,category)"
        )
        .eq("cart_id", cart.id);
      ...
      <section className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <div className="rounded-2xl border bg-white/90 p-6 shadow-sm">
            ...
            <Link
              href={buyNow ? `/checkout/success?buyNow=1` : "/checkout/success"}
          ...
          <div className="rounded-2xl border bg-white/90 p-6 shadow-sm">
            ...
            <div className="mt-5 rounded-xl border bg-white p-4">
              <p className="text-xs font-semibold text-slate-700">
                {buyNow ? "Buying now" : "Items in cart"}
              </p>
              <p className="mt-1 text-sm text-slate-600">
